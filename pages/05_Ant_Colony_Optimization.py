...
with tabs[2]:
    ...
    if st.button("ACO ausführen"):
        ...
        def update_progress(current, total, best_score):
            ...
            progress_text.text(f"Iteration {current}/{total} – Aktueller bester Score: {best_score:.2f}")
        ...
        columns = [f"Tag {d+1}_{label}" for d in range(D) for label in ["FS", "SS", "NS"][:S]]
        ...
        for d in range(D):
            ...
            if d % 2 == 0:
                shapes.append(dict(
                    ...
                    fillcolor="rgba(230,230,230,0.6)", line_width=0, layer="below"
                ))
